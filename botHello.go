...
func (t *TeleAPI) SendMessage(chatID int64, name string, text string) {
	...
	case "pong":
		send(t.sendMsg, `"text": "ping"`)
	case "hi", "hello", "привет", "Привет":
		send(t.sendMsg, `"text": "Hello"`)
	case "фото", "скинь фото":
		send(t.sendMsg, `"text": "пока что я не умею"`)
	case "я тебя люблю", "Я тебя люблю", "Я люблю тебя", "яс люблю тебя":
		send(t.sendMsg, `"text": "Изменять Витале нельзя!"`)
	case "бот", "к ноге":
